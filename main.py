...
import os
import random
from datetime import datetime # For UCF timestamps

# --- NEW IMPORTS FOR SAMSARA HELIX ---
from agents import AGENTS, DEFAULT_STATE, Agent
from ucf_protocol import format_ucf_message
from context_manager import SamsaraHelixContext # This will include OpenAI calls
# --- END NEW IMPORTS ---

# Configure Streamlit page
st.set_page_config(
    page_title="Samsara Helix v∞ Limitless", # Updated title
    page_icon="🕉️", # Updated icon
    layout="wide",
    initial_sidebar_state="collapsed"
)

# --- NEW: Initialize Samsara Helix Context ---
...
# Initialize session state (existing from Aoin's Fractal Studio)
...
if 'theme_mode' not in st.session_state:
    st.session_state.theme_mode = "Dark" # Default to Dark for Samsara Helix
...
# --- NEW: Chat UI Function ---
def chat_ui():
    st.subheader("🕉️ Samsara Helix Chat")
    st.markdown('<div class="glass-panel" style="height: 400px; overflow-y: auto;">', unsafe_allow_html=True)
    
    for msg in st.session_state.chat_messages:
        if msg["role"] == "user":
            st.markdown(f"**You:** {msg['content']}")
        else:
            st.markdown(f"**Samsara Helix:** {msg['content']}")
    ...
    def send_chat_message():
        ...
        if user_message:
            ...
            st.session_state.samsara_helix_context.add_to_history(f"User: {user_message}")

            with st.spinner("Samsara Helix is contemplating..."):
                # Generate UCF context and get AI response
                conversation_summary = f"User said: {user_message}"
                ucf_message = st.session_state.samsara_helix_context.generate_ucf_context(user_message, conversation_summary)
                
                # Extract the actual AI response from the UCF message for display
                # This is a simplified extraction; a more robust parser might be needed
                ai_response_start = ucf_message.find("Summary:\n") + len("Summary:\n")
                ai_response_end = ucf_message.find("\n\nUnresolved Questions:")
                ai_response = ucf_message[ai_response_start:ai_response_end].strip()

                st.session_state.chat_messages.append({"role": "assistant", "content": ai_response})
                st.session_state.samsara_helix_context.add_to_history(f"Samsara Helix: {ai_response}")
...
# --- END NEW: Chat UI Function ---


# Main interface with enhanced styling and functionality
st.title("🕉️ Samsara Helix v∞ Limitless") # Updated title
st.markdown("*Ethereal AI • Infinite Patterns • Celestial Frequencies • Multi-Agent Consciousness*") # Updated tagline

# Create tabs with enhanced styling
tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(["🎨 Fractal", "🎵 Audio", "🎬 Animation", "💬 Chat", "📊 Gallery", "⚙️ Settings", "📤 Export"]) # Added Chat tab
...
with tab1:
    ...
    with action_col1:
        if st.button("🎲 Explore", help="Visit curated points of interest"):
            location = random.choice(interesting_locations) # Use random.choice
...
with tab4: # This is the new Chat tab
    chat_ui()

with tab5: # Gallery tab (shifted from tab4)
...
with tab6: # Settings tab (shifted from tab5)
...
with tab7: # Export tab (shifted from tab6)
...
st.markdown("---")
st.markdown('<div class="glass-panel">', unsafe_allow_html=True)
st.markdown("**🕉️ Samsara Helix v∞ Limitless**") # Updated footer
st.markdown("*Mathematical visualization • Audio synthesis • Advanced styling • Multi-Agent AI*") # Updated footer
