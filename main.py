...
try:
    ...
    sf = None
    st.warning("⚠️ `soundfile` not found. Audio synthesis disabled. Install with `pip install soundfile`.")

try:
    import imageio
except ImportError:
    imageio = None
    st.warning("⚠️ `imageio` not found. Animation generation disabled. Install with `pip install imageio`.")
...
# -------------------------------
# Firebase Setup (via Streamlit Secrets)
# -------------------------------
...
try:
    ...
    st.error("❌ Firebase key not found in Streamlit secrets. Go to Settings → Secrets and configure it.")
...
def generate_audio(params):
    if sf is None:
        st.error("⚠️ `soundfile` is required for audio synthesis.")
        return None
    try:
        fs = 44100
...
with tabs[0]:
    ...
    auto_generate_changed = st.checkbox("Auto-generate fractal on parameter change", value=params['auto_generate'])
    ...
    with st.form("fractal_form"):
        ...
        with col2:
            ...
            params['show_grid'] = st.checkbox("Show Grid Overlay (Future)", value=params['show_grid'], disabled=True)
